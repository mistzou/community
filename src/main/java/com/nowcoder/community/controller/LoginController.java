...
public class LoginController implements CommunityConstant {
    ...
    public String activation(Model model, @PathVariable("userId")int userId,@PathVariable("code")String code){
        int result = userService.activationUtil(userId,code);
        if(result== ACTIVATION_SUCCESS){
            model.addAttribute("msg","激活成功,您的账号已经可以正常使用了！");
            model.addAttribute("target","/login");
        }else if (result==ACTIVATION_REPEAT){
            model.addAttribute("msg","无效操作，您的账号无需重复激活！");
            model.addAttribute("target","/index");
        }else {
            model.addAttribute("msg","无效操作，您的账号无需重复激活！");
            model.addAttribute("target","/index");
    ...
    public void getKaptcha(HttpSession session, HttpServletResponse response){
        ...
        try {
            ...
            logger.error("登录生成验证码失败！"+e.getMessage());
        }
    }
    ...
    public String login(Model model,HttpServletResponse response,HttpSession session,
        ...
        int expiredSeconds = remember?REMEBER_EXPIRED_SECONDS:DEFAULT_EXPIRED_SECONDS;
